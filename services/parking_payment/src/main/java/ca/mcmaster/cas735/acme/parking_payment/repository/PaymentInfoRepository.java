...
public interface PaymentInfoRepository extends JpaRepository<PaymentInfo, Long> {
    ...
    @Query(value = "SELECT COUNT(l.bill) FROM t_payment_info l WHERE l.product_name = 2 and l.payment_status = 2",
            nativeQuery = true)
    Integer countTransponderSales();

    @Query(value = "SELECT SUM(l.bill) FROM t_payment_info l WHERE l.product_name = 2 and l.payment_status = 2",
            nativeQuery = true)
    Integer SumTransponderSales();

    @Query(value = "SELECT SUM(l.bill) FROM t_payment_info l WHERE l.product_name = 1 and l.payment_status = 2",
            nativeQuery = true)
    Integer SumParkingSales();

    @Query(value = "SELECT SUM(l.bill) FROM t_payment_info l WHERE l.payment_status = 2",
            nativeQuery = true)
    Integer SumSales();
}
